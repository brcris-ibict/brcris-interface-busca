...
window.PainelGraficos = (function () {
  ...
  function getChart(id) {
    ...
    if (!chart) {
      chart = echarts.init(element, getTheme() === "dark" ? "dark" : null, {
        renderer: "svg",
      });
    }
  ...
  function render(data) {
    ...
    if (data.length === 0) {
      chartInstances.forEach((chart, id) => {
        ...
        if (el)
          el.innerHTML = `<div class="d-flex align-items-center justify-content-center h-100 w-100 text-muted">Sem dados para exibir</div>`;
        clearInsight(id);
      });
      const tbody = document.querySelector("#journal-table tbody");
      if (tbody)
        tbody.innerHTML = `<tr><td colspan="2" class="text-center text-muted p-3">Nenhuma publicação encontrada.</td></tr>`;
      return;
    } else {
  ...
  function renderG1_EvolucaoAnual(data) {
    ...
    setInsight(
      "chart-evolution",
      `O maior volume no período selecionado ocorreu em <strong>${maxYear}</strong>, com ${maxCount.toLocaleString("pt-BR")} registros disponiveis.`,
    );
  }
  ...
  function renderG5_EvolucaoTipo100(data) {
    ...
    chart.setOption(
      {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            ...
            params.forEach((p) => {
              if (p.data.value > 0) {
                const valPct = p.data.value.toLocaleString("pt-BR", {
    ...
    setInsight(
      ...
      `A visualização em % permite entender como a proporção dos tipos de documentos mudou ao longo do tempo, isolando o crescimento do volume total.`,
    );
  }
  ...
  function renderG2_ComposicaoTipo(data) {
    ...
    chart.setOption(
      {
        ...
        yAxis: {
          ...
          data: types,
          axisLine: { lineStyle: { color: palette.border } },
          axisLabel: { color: palette.muted, width: 130, overflow: "truncate" },
        },
        series: [
          {
            type: "bar",
            data: vals.map((v, idx) => ({
              value: v,
              itemStyle: { color: getTypeColor(types[idx]) },
            })),
    ...
    if (topT) {
      const pct = ((topT[1] / data.length) * 100).toLocaleString("pt-BR", {
        maximumFractionDigits: 1,
      });
      setInsight(
        "chart-types",
        `<strong>${topT[0]}s</strong> representam ${pct}% das publicações no recorte selecionado.`,
      );
    }
  }
  ...
  function renderG3_Idioma(data) {
    ...
    const counts = data.reduce((acc, d) => {
      d.languages.forEach((l) => (acc[l] = (acc[l] || 0) + 1));
      return acc;
    }, {});
    const sorted = Object.entries(counts).sort((a, b) => a[1] - b[1]);
  ...
  function renderG4_InstituicoesTop10(data) {
    ...
    const counts = data.reduce((acc, d) => {
      d.institutions.forEach((inst) => (acc[inst] = (acc[inst] || 0) + 1));
      return acc;
    }, {});
    ...
    const insts = reverseSorted.map((i) =>
      i[0].split("(")[1]
        ? i[0].split("(")[1].replace(")", "")
        : i[0].substring(0, 25),
    );
    ...
    chart.setOption(
      {
        ...
        tooltip: {
          ...
          axisPointer: { type: "shadow" },
          formatter: (params) =>
            `<strong>${reverseSorted[params[0].dataIndex][0]}</strong><br/>Associações: ${params[0].value}`,
        },
        ...
        yAxis: {
          ...
          data: insts,
          axisLine: { lineStyle: { color: palette.border } },
          axisLabel: { color: palette.muted, width: 110, overflow: "truncate" },
        },
        series: [
          {
            type: "bar",
    ...
    const top5Total = sorted
      .slice(0, 5)
      .reduce((sum, item) => sum + item[1], 0);
    const allAssoc = Object.values(counts).reduce((sum, item) => sum + item, 0);
    const top5Pct = ((top5Total / allAssoc) * 100).toLocaleString("pt-BR", {
      maximumFractionDigits: 1,
    });
    setInsight(
      "chart-institutions",
      `As cinco instituições com maior presença concentram <strong>${top5Pct}%</strong> das ${allAssoc.toLocaleString("pt-BR")} associações do recorte.`,
    );
  }
  ...
  function renderTabelaPeriodicos(data) {
    const tbody = document.querySelector("#journal-table tbody");
    if (!tbody)
      throw new Error("tbody da tabela de periódicos não encontrado.");

    const artigos = data.filter((d) => d.type === "Artigo" && d.journal);
    const counts = artigos.reduce((acc, d) => {
      acc[d.journal] = (acc[d.journal] || 0) + 1;
      return acc;
    }, {});
    const sorted = Object.entries(counts)
      .sort((a, b) => b[1] - a[1])
    ...
    tbody.innerHTML = sorted
      .map(([journal, count], index) => {
        const pct =
          ((count / allArticles) * 100).toLocaleString("pt-BR", {
            maximumFractionDigits: 1,
          }) + "%";
        return `<tr>
