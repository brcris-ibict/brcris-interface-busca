...
window.PainelFiltros = (function () {
  ...
  function renderFiltrosUI() {
    ...
    const anos = [...new Set(_baseData.map(d => d.year))].sort((a, b) => a - b);
  ...
  function removeFilterTag(type, val) {
    ...
    else if (type === 'type') _state.types = _state.types.filter(t => t !== val);
    else if (type === 'lang') _state.languages = _state.languages.filter(l => l !== val);
    
    // Atualiza selects visuais para período
    if (type === 'yearFrom') document.getElementById("filter-year-from").value = "";
  ...
  function applyAndNotify() {
    ...
    const filtered = _baseData.filter(d => {
      ...
      // AND (OR types)
      if (_state.types.length > 0 && !_state.types.includes(d.type)) return false;
      // AND (OR languages) - intersecção entre idiomas selecionados e idiomas do registro
      if (_state.languages.length > 0 && !d.languages.some(l => _state.languages.includes(l))) return false;
  ...
  function updateUI(filteredCount) {
    const hasFilters = _state.yearFrom !== null || _state.yearTo !== null || _state.types.length > 0 || _state.languages.length > 0;
    ...
    if (tagsContainer) {
      ...
      const createTag = (label, type, val) => {
        ...
        `;
        div.querySelector("button").addEventListener("click", () => removeFilterTag(type, val));
        return div;
      };

      if (_state.yearFrom !== null) tagsContainer.appendChild(createTag(`De: ${_state.yearFrom}`, 'yearFrom', _state.yearFrom));
      if (_state.yearTo !== null) tagsContainer.appendChild(createTag(`Até: ${_state.yearTo}`, 'yearTo', _state.yearTo));
      _state.types.forEach(t => tagsContainer.appendChild(createTag(`${t}`, 'type', t)));
      _state.languages.forEach(l => tagsContainer.appendChild(createTag(`${l}`, 'lang', l)));
